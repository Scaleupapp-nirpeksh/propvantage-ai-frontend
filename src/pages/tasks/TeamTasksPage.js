...
const TeamTasksPage = () => {
  ...
  // Helper: normalise each workload item regardless of API shape
  // Supports { user, totalTasks, overdueTasks } and { userId, total, overdue } variants
  const normaliseWorkload = (w) => ({
    memberId: w._id || w.userId?._id || w.userId,
    firstName: w.user?.firstName || w.userId?.firstName || w.firstName || '',
    lastName: w.user?.lastName || w.userId?.lastName || w.lastName || '',
    total: w.totalTasks ?? w.total ?? 0,
    overdue: w.overdueTasks ?? w.overdue ?? 0,
    critical: w.criticalTasks ?? w.critical ?? 0,
  });

  const teamMembers = workload.map((w) => {
    const n = normaliseWorkload(w);
    return { _id: n.memberId, firstName: n.firstName, lastName: n.lastName };
  });
  ...
  const columns = [
    ...
    },
    {
      ...
      label: 'Due',
      width: 100,
      render: (val) => val ? formatDate(val, { format: 'medium' }) : '-',
    },
  ];
    ...
    <Box>
      ...
      {workload.length > 0 && (
        <Box sx={{ display: 'flex', gap: 2, overflowX: 'auto', pb: 2, mb: 2 }}>
          {workload.map((w) => {
            const { memberId, firstName, lastName, total, overdue, critical } = normaliseWorkload(w);
            const name = formatName(firstName, lastName, { format: 'full' });
            const initials = formatName(firstName, lastName, { format: 'initials' });
            ...
            return (
              ...
              >
                ...
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                  <Chip label={`${total} total`} size="small" sx={{ height: 20, fontSize: '0.625rem' }} />
                  {overdue > 0 && (
                    <Chip label={`${overdue} overdue`} size="small" sx={{ height: 20, fontSize: '0.625rem', bgcolor: alpha(theme.palette.error.main, 0.1), color: 'error.main' }} />
                  )}
                  {critical > 0 && (
                    <Chip label={`${critical} critical`} size="small" sx={{ height: 20, fontSize: '0.625rem', bgcolor: alpha(theme.palette.warning.main, 0.1), color: 'warning.main' }} />
                  )}
                </Box>
              </Paper>
            );
